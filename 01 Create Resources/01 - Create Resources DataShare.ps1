...
if(-not $SecretCheck1)
    ...
    #$Secret1 = ConvertTo-SecureString -String 'DefaultEndpointsProtocol=https;AccountName=cricestridesstor;AccountKey=cej/aEOwD+m2aXMY75Z17uY3CLirEHlMvfds2sY6nKaEYW+fISV8Iy15rJi713ye9wsTBM27hg8xrVTdiQTPJQ==;EndpointSuffix=core.windows.net' -AsPlainText -Force
...
<#
Write-Host "The Azure Key Vault secret 3 creation was started " $startTime

$SecretCheck3 = Get-AzKeyVaultSecret -VaultName $akvname -Name $akvsecret3 -ErrorAction SilentlyContinue
if(-not $SecretCheck3)
    {
    Write-Host "Azure Key Vault secret '$akvsecret3' doesn't exist and will be created"
    #$Secret1 = ConvertTo-SecureString -String 'DefaultEndpointsProtocol=https;AccountName=cricestridesstor;AccountKey=cej/aEOwD+m2aXMY75Z17uY3CLirEHlMvfds2sY6nKaEYW+fISV8Iy15rJi713ye9wsTBM27hg8xrVTdiQTPJQ==;EndpointSuffix=core.windows.net' -AsPlainText -Force
    $Secret3 = Read-Host -Prompt "Enter text for AKV secret '$akvsecret3'" -AsSecureString
    Set-AzKeyVaultSecret -VaultName $akvname -Name $akvsecret3 -SecretValue $Secret3
    }
else 
    {Write-Host "Azure Key Vault '$akvsecret3' already created"}
#>
